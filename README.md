...
- **🎛️ Dynamic Repo Health Matrix**: A multi-dimensional scorecard widget in the report view that breaks down 6 core metrics-Security, Accessibility, Test Coverage, Code Cleanliness, Efficiency, and Architecture-all dynamically graded and generated by the AI from 0-100% based on the repository's codebase.
...
RepoGuard includes a robust unit test suite built with **Vitest**, **JSDOM**, and **React Testing Library**. It covers complex component rendering, state transitions, API mocking, and property-based testing (via `fast-check`) for core utilities and UI components without touching production code.

To run the test suite and view coverage metrics:
